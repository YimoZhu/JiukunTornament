            ...
            for i in range(maxLen):
                if runtime['isOpen_A001.PSE']:
                    ...
                    try:
                        ...
                        broker_stub_ind.new_order(api.TraderRequest_new_order('A001.PSE',0,0,buy_new_order[1],buy_new_order[0]))
                    except:
                        pass
                    try:
                        sell_new_order = sell_new_orders[i]
                        broker_stub_ind.new_order(api.TraderRequest_new_order('A001.PSE',1,1,sell_new_order[1],sell_new_order[0]))
                    except:
                        pass
                else:
                    try:
                        buy_new_order = buy_new_orders[i]
                        broker_stub_ind.new_order(api.TraderRequest_new_order('A001.PSE',0,1,buy_new_order[1],buy_new_order[0]))
                    except:
                        pass
                    try:
                        sell_new_order = sell_new_orders[i]
                        broker_stub_ind.new_order(api.TraderRequest_new_order('A001.PSE',1,0,sell_new_order[1],sell_new_order[0]))
                    except:
                        pass
            #//进行撤单
        ...
        if runtime['isDigested_A002.PSE'] == 0:
            ...
            for i in range(maxLen):
                if runtime['isOpen_A002.PSE']:
                    ...
                    try:
                        buy_new_order = buy_new_orders[i]
                        broker_stub_ind.new_order(api.TraderRequest_new_order('A002.PSE',0,0,buy_new_order[1],buy_new_order[0]))
                    except:
                        pass
                    try:
                        sell_new_order = sell_new_orders[i]
                        broker_stub_ind.new_order(api.TraderRequest_new_order('A002.PSE',1,1,sell_new_order[1],sell_new_order[0]))
                    except:
                        pass
                else:
                    try:
                        buy_new_order = buy_new_orders[i]
                        broker_stub_ind.new_order(api.TraderRequest_new_order('A002.PSE',0,1,buy_new_order[1],buy_new_order[0]))
                    except:
                        pass
                    try:
                        sell_new_order = sell_new_orders[i]
                        broker_stub_ind.new_order(api.TraderRequest_new_order('A002.PSE',1,0,sell_new_order[1],sell_new_order[0]))
                    except:
                        pass
            #//进行撤单
            for order_id in order_to_cancel:
        ...
        if runtime['isDigested_B001.PSE'] == 0:
            ...
            for i in range(maxLen):
                if runtime['isOpen_B001.PSE']:
                    ...
                    try:
                        buy_new_order = buy_new_orders[i]
                        broker_stub_ind.new_order(api.TraderRequest_new_order('B001.PSE',0,0,buy_new_order[1],buy_new_order[0]))
                    except:
                        pass
                    try:
                        sell_new_order = sell_new_orders[i]
                        broker_stub_ind.new_order(api.TraderRequest_new_order('B001.PSE',1,1,sell_new_order[1],sell_new_order[0]))
                    except:
                        pass
                else:
                    try:
                        buy_new_order = buy_new_orders[i]
                        broker_stub_ind.new_order(api.TraderRequest_new_order('B001.PSE',0,1,buy_new_order[1],buy_new_order[0]))
                    except:
                        pass
                    try:
                        sell_new_order = sell_new_orders[i]
                        broker_stub_ind.new_order(api.TraderRequest_new_order('B001.PSE',1,0,sell_new_order[1],sell_new_order[0]))
                    except:
                        pass
            #//进行撤单
            for order_id in order_to_cancel:
...
if __name__ == '__main__':
    ...
    with Manager() as mgr:
        ...
        p_control = Process(target = tradingObjects.positions_control_notNet_looped, args = (runtime,))
